...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const auto& Pair : FTopDownRPGGameplayTags::Get().DamageTypesToResistances)
	{
		...
		checkf(TopDownRPGDamageStatics().TagsToCaptureDefs.Contains(ResistanceTag),TEXT("TagsToCaptureDefs doesn't contain Tag : [%s] in ExecCala_Damage"),*ResistanceTag.ToString());
		const FGameplayEffectAttributeCaptureDefinition CaptureDef = TopDownRPGDamageStatics().TagsToCaptureDefs[ResistanceTag];
