...
APointCollection::APointCollection()
{
	...
	Pt_6 = CreateDefaultSubobject<USceneComponent>("Pt61");
...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GroundLocation, int32 NumPoint, float YawOverride)
{
	checkf(ImmutablePts.Num() >= NumPoint, TEXT("Attempted to access ImmutablePts out of bounds."));

	TArray<USceneComponent*> ArrayCopy;
	for (USceneComponent* Pt : ImmutablePts)
	{
		if (ArrayCopy.Num() >= NumPoint)
		{
			return ArrayCopy;
		}
		if (Pt != Pt_0)
		...
		UTopDownRPGAbilitySystemLibrary::GetLivePlayerWithRadius(this, IgnoreActors, TArray<AActor*>(), 1500.f, GetActorLocation());
